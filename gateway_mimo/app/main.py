...
MIMO_PAYG_BASE_URL = os.getenv("MIMO_PAYG_BASE_URL", "https://api.xiaomimimo.com/anthropic").rstrip("/")
MIMO_TP_REGION = os.getenv("MIMO_TP_REGION", "cn").strip().lower()
MIMO_TP_BASE_URL_CN = os.getenv(
    "MIMO_TP_BASE_URL_CN",
    "https://token-plan-cn.xiaomimimo.com/anthropic",
).rstrip("/")
MIMO_TP_BASE_URL_SGP = os.getenv(
    "MIMO_TP_BASE_URL_SGP",
    "https://token-plan-sgp.xiaomimimo.com/anthropic",
).rstrip("/")
MIMO_TP_BASE_URL_AMS = os.getenv(
    "MIMO_TP_BASE_URL_AMS",
    "https://token-plan-ams.xiaomimimo.com/anthropic",
).rstrip("/")
MODEL_PRIMARY = os.getenv("MODEL_PRIMARY", "mimo-v2.5")
MODEL_FAST = os.getenv("MODEL_FAST", "mimo-v2-flash")
...
# Anthropic compatibility allowlist.
TOP_LEVEL_ALLOWLIST = {
    "model",
    "max_tokens",
    "messages",
...
TP_BASE_URL_BY_REGION = {
    "cn": MIMO_TP_BASE_URL_CN,
    "sgp": MIMO_TP_BASE_URL_SGP,
    "ams": MIMO_TP_BASE_URL_AMS,
}

if MIMO_TP_REGION not in TP_BASE_URL_BY_REGION:
    print(f"[gateway startup] invalid MIMO_TP_REGION={MIMO_TP_REGION!r}; fallback=cn")
    MIMO_TP_REGION = "cn"

app = FastAPI(title="Excel Claude -> Mimo Gateway", version="1.0.0")
...
print(
    "[gateway startup] provider=mimo "
    f"payg_base={_mask_url(MIMO_PAYG_BASE_URL)} "
    f"tp_region_default={MIMO_TP_REGION} "
    f"tp_base_cn={_mask_url(MIMO_TP_BASE_URL_CN)} "
    f"tp_base_sgp={_mask_url(MIMO_TP_BASE_URL_SGP)} "
    f"tp_base_ams={_mask_url(MIMO_TP_BASE_URL_AMS)} "
    f"model_primary={MODEL_PRIMARY} model_fast={MODEL_FAST}"
)
...
def _resolve_upstream_key(req: Request) -> str:
    ...
    token = MIMO_API_KEY or _extract_incoming_token(req)
    if not token:
...
def _resolve_tp_region(req: Request) -> str:
    override_region = req.headers.get("x-mimo-tp-region", "").strip().lower()
    if not override_region:
        return MIMO_TP_REGION
    if override_region not in TP_BASE_URL_BY_REGION:
        raise HTTPException(
            status_code=400,
...
def _resolve_upstream_base_url(req: Request, upstream_key: str) -> Tuple[str, str]:
    # Validate region header whenever provided, even for PAYG requests.
    region = _resolve_tp_region(req)
    if upstream_key.startswith("sk-"):
        return MIMO_PAYG_BASE_URL, "payg"
    if upstream_key.startswith("tp-"):
        return TP_BASE_URL_BY_REGION[region], f"token-plan:{region}"
    # _resolve_upstream_key already enforces prefixes; keep a safe fallback.
    raise HTTPException(status_code=401, detail="Invalid API key prefix, expected sk- or tp-")
...
async def create_message(req: Request):
    ...
    upstream_base_url, upstream_route = _resolve_upstream_base_url(req, upstream_key)
    ...
    print(
        "[gateway route] "
        f"route={upstream_route} upstream_url={_mask_url(upstream_url)} "
        f"key_prefix={'tp-' if upstream_key.startswith('tp-') else 'sk-'}"
    )
