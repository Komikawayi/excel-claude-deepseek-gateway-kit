...
_kimi_coding_base_url = os.getenv("KIMI_CODING_BASE_URL", "").strip()
_kimi_payg_base_url = os.getenv("KIMI_PAYG_BASE_URL", "").strip()
CODINGPLAN_BASE_URL = (
...
PAYG_MODEL_PRIMARY = os.getenv("PAYG_MODEL_PRIMARY", os.getenv("MODEL_PRIMARY", "kimi-k2.6")).strip()
PAYG_MODEL_FAST = os.getenv("PAYG_MODEL_FAST", os.getenv("MODEL_FAST", PAYG_MODEL_PRIMARY or "kimi-k2.6")).strip()
CODINGPLAN_MODEL = os.getenv("CODINGPLAN_MODEL", "kimi-for-coding").strip() or "kimi-for-coding"
...
# Kimi Anthropic compatibility allowlist.
TOP_LEVEL_ALLOWLIST = {
    "model",
    "max_tokens",
    "messages",
...
SUPPORTED_CONTENT_BLOCK_TYPES = {"text", "thinking", "tool_use", "tool_result", "image", "image_url"}
...
app = FastAPI(title="Excel Claude -> Kimi Gateway", version="1.5.1")
...
print(
    "[gateway startup] provider=kimi "
    f"coding_base={_mask_url(CODINGPLAN_BASE_URL)} payg_base={_mask_url(PAYG_BASE_URL)} "
    f"payg_model_primary={PAYG_MODEL_PRIMARY} payg_model_fast={PAYG_MODEL_FAST} "
    f"coding_model={CODINGPLAN_MODEL}"
)
...
def _extract_incoming_token(req: Request) -> str:
    ...
    for header_name in ("x-api-key", "api-key"):
        candidate = req.headers.get(header_name, "").strip()
        if candidate:
            return candidate
    return ""


def _classify_key_prefix(api_key: str) -> str:
...
def _resolve_upstream_key(req: Request) -> Tuple[str, str]:
    # Priority: KIMI_API_KEY env > incoming request key.
    if KIMI_API_KEY:
        key_class = _classify_key_prefix(KIMI_API_KEY)
        return KIMI_API_KEY, key_class
    incoming = _extract_incoming_token(req)
    if incoming:
        key_class = _classify_key_prefix(incoming)
        return incoming, key_class
    # Compatibility fallback when request key is absent.
    if UPSTREAM_API_KEY:
        key_class = _classify_key_prefix(UPSTREAM_API_KEY)
        return UPSTREAM_API_KEY, key_class
    raise HTTPException(status_code=401, detail="No API key available (env or incoming token)")


def _resolve_upstream_route(req: Request) -> Tuple[str, str, str, str]:
    upstream_key, key_class = _resolve_upstream_key(req)
    if key_class == "sk-kimi-*":
        return upstream_key, key_class, "codingplan", CODINGPLAN_BASE_URL
    return upstream_key, key_class, "payg", PAYG_BASE_URL


def _route_model(model_id: str) -> str:
    value = (model_id or "").strip()
    if not value:
        return PAYG_MODEL_PRIMARY

    alias_map = {
        ALIAS_SONNET: PAYG_MODEL_PRIMARY,
        ALIAS_SONNET_VERSIONED: PAYG_MODEL_PRIMARY,
        ALIAS_OPUS: PAYG_MODEL_PRIMARY,
        ALIAS_OPUS_VERSIONED: PAYG_MODEL_PRIMARY,
        ALIAS_HAIKU: PAYG_MODEL_FAST,
        ALIAS_HAIKU_VERSIONED: PAYG_MODEL_FAST,
        "sonnet": PAYG_MODEL_PRIMARY,
        "opus": PAYG_MODEL_PRIMARY,
        "haiku": PAYG_MODEL_FAST,
        PAYG_MODEL_PRIMARY: PAYG_MODEL_PRIMARY,
        PAYG_MODEL_FAST: PAYG_MODEL_FAST,
    }
    return alias_map.get(value, PAYG_MODEL_PRIMARY)
...
async def create_message(req: Request):
    upstream_key, key_class, route_kind, route_base_url = _resolve_upstream_route(req)
    print(f"[gateway route] kind={route_kind} key_class={key_class}")
    ...
    if route_kind == "codingplan":
        body["model"] = CODINGPLAN_MODEL
    else:
        body["model"] = _route_model(str(body.get("model", "")))
    req_summary_raw = dict(raw_body)
    req_summary_raw["model"] = body["model"]
    req_summary = _build_request_shape_summary(req_summary_raw, body["model"])
    ...
    upstream_url = f"{route_base_url}/v1/messages"
