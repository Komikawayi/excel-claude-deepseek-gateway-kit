...
网关对外保持 Anthropic 接口形态（`/v1/models`、`/v1/messages`），对内可路由到 DeepSeek / Kimi / MiMo。
...
| Claude 别名 | DeepSeek | Kimi | MiMo |
|---|---|---|---|
| `opus` | `deepseek-v4-pro` | `kimi-k2.6` | `mimo-v2.5-pro` |
| `sonnet` | `deepseek-v4-flash` | `kimi-k2.5` | `mimo-v2.5` |
| `haiku`（兼容） | 映射到 sonnet 档 | 映射到 sonnet 档 | 映射到 sonnet 档 |
...
This project keeps an Anthropic-compatible surface (`/v1/models`, `/v1/messages`) and forwards requests to DeepSeek, Kimi, or MiMo.
