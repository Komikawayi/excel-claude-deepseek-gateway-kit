...
```powershell
cd E:\gateway_komi\gateway_unified
powershell -ExecutionPolicy Bypass -File .\run-gateway.ps1 -Install
```
...
### 2.2 NPM 包装方式（对 Node 用户友好）

说明：网关本体依然是 Python，`npm` 只是命令包装层，不是 Node 重写版本。
